odoo.define('invoice_xunnel.l10n_mx_edi_vendor_bills', function (require) {
    ...
    var attachXmlsWizard = FieldChar.extend({
        template: 'xunnel_attach_xmls_template',
        events: {
            ...
            'dragover #dragandrophandler': function (e) {
                e.stopPropagation();
                e.preventDefault();
                $(e.currentTarget).removeClass('dnd_out').addClass(
                    'dnd_inside');
            },
            ...
            // Removes the file with a click over the respective file content
            // in page
            ...
            // Executes the respective actions for each case in the exceptions
            // after press button save
            'click #alertscontent .dnd-alert-button': function (e) {
                ...
                if (type === 'remove') {
                    ...
                    self._rpc({
                        ...
                        args: [this.alerts_in_queue.alertHTML[filekey].xml64,
                            filekey],
                        context: self.getParent().state.context,
                    }).then(function () {
                        self.sendErrorToServer(
                            self.alerts_in_queue.alertHTML[filekey].xml64,
                            filekey, 'check_xml');
                    });
                } else if (type === 'tryagain') {
                    this.sendErrorToServer(
                        this.alerts_in_queue.alertHTML[filekey].xml64, filekey,
                        'check_xml');
                } else if (type === 'forcesave') {
                    self.getParent().state.context = _.extend(
                        self.getParent().state.context, {'force_save': true});
                    self.sendErrorToServer(
            ...
            'click #dndfooter button#save': function (e) {
                ...
                } else if (Object.keys(this.files).length > 1 &&
                    this.getParent().state.context.active_ids) {
                    this.do_warn(_t(
                        'You cannot attach more than one xml to an invoice'));
                } else {
                    this.$el.find("#dragandrophandler").hide();
                    this.$el.find(
                        '#dndfooter button#save').attr('disabled', true);
                    this.$el.find("#filescontent").find(
                        '.xml_cont').removeClass('xml_cont_hover');
        ...
        start: function () {
            ...
            $(document).on('dragenter', function (e) {
                ...
                e.preventDefault();
                handler.removeClass(
                    'dnd_inside dnd_normal').addClass('dnd_out');
            });
            ...
            $(document).on('drop', function (e) {
                ...
                e.preventDefault();
                handler.removeClass(
                    'dnd_out dnd_inside').addClass('dnd_normal');
            });
        ...
        handleFileUpload: function (files) {
            ...
            /* Creates the file element in the DOM and shows alerts wheter the
                * extension file is not the correct one or the file is already
                * uploaded */
            var self = this;
            if (self.uploading_files) {
                ...
                $.each(files, function (i, file) {
                    if (file.type !== 'text/xml') {
                        ...
                        var newelement = $(
                            '<div class="xml_cont xml_cont_hover" title="'+
                            file.name +'"><img class="xml_img" height="100%"'+
                            ' align="left" hspace="5"/><p>'+file.name+
                            '</p><div class="remove_xml">&times;</div>' +
                            '</div>').css('opacity', '0');
                ...
                if (wrong_files.length > 0) {
                    alert_message += _t(
                        '<strong>Info!</strong> You only can upload XML '+
                        'files.<br>') + wrong_files.join(
                ...
                if (files_used.length > 0) {
                    ...
                    alert_message += _t(
                        '<strong>Info!</strong> Some files are already '+
                        'loaded.<br>') + files_used.join(" <b style='"+
                        "font-size:15px;font-wight:900;'>&#8226;</b> ");
                }
                if (alert_message !== '') {
                    self.$el.find("#alertscontent").html(
                        '<div class="alert alert-warning dnd-alert">' +
                        '<a href="#" class="close" data-dismiss="alert" '+
                        'aria-label="close">&times;</a>' + alert_message +
                        '</div>');
                }
        ...
        sendFileToServer: function (files) {
            ...
            /* Sends each base64 file string to the back-end server to create
                * the invoices */
            var self = this;
        ...
        createdCorrectly: function (key) {
            ...
            /* Colors the files content in the DOM when the invoice is created
                * with that XML */
            var self = this;
        ...
        handleFileWrong: function (wrongfiles) {
            ...
            this.alerts_in_queue = {'alertHTML': {},
            ...
            $.each(wrongfiles, function (key, file) {
                if ('cfdi_type' in file) {
                    ...
                    var alertelement = $(
                        '<div tag="'+ key +'" class="alert alert-'+
                        alert_parts.alerttype +' dnd-alert">' +
                        alert_parts.errors + '<div>' + alert_parts.buttons+
                        _t('<span>Wrong File: <span '+
                           'class="index-alert"></span>') + '/' +
                        self.alerts_in_queue.total +
                        '<b style="font-size:15px;font-wight:900;">&#8226;'+
                        '</b> ' + key + '</span></div></div>');
                    // eslint-disable-next-line max-len
                    self.alerts_in_queue.alertHTML[key] = {'alert': alertelement, 'xml64': file.xml64};
                }
                if (self.alerts_in_queue.total > 0 &&
                        self.alerts_in_queue.total ===
                        Object.keys(self.alerts_in_queue.alertHTML).length) {
                    self.nextWrongAlert();
                }
            });
        },
        prepareWrongAlert: function (key, data) {
            ...
            /* Prepares the buttons and message the invoice alert exception
                * will contain */
            /*eslint-disable */
            var self = this;
            var errors = '';
            ...
            if (able_buttons.includes('remove')) {
                buttons += _t(
                    '<button class="dnd-alert-button" tag="remove">Remove XML</button>');
            } else if (able_buttons.includes('supplier') &&
                    !able_buttons.includes('remove')) {
            ...
            } else if (able_buttons.includes('customer') &&
                    !able_buttons.includes('remove')) {
                buttons += _t('<button class="dnd-alert-button" tag="remove">Remove XML</button>') +
        ...
        wrongMsgXml: function (file, able_buttons) {
            ...
            $.each(file, function (ikey, val) {
                if (ikey !== 'supplier' && ikey !== 'customer' && ikey !== 'xml64' && ikey !== 'invoice_not_found' && !able_buttons.includes('remove')) {
        ...
        sendErrorToServer: function (xml64, key, function_def) {
            ...
            /* Sends again the base64 file string to the server to tries to
                * create the invoice, or sends the partner data to create him
                * if does not exist */
            var self = this;
            var xml_file = {};
            xml_file[key] = xml64;
            ...
            self._rpc({
                ...
                $.each(invoices, function (rkey, result) {
                    var alertobj = self.$el.find(
                        '#alertscontent div[tag="'+rkey+'"].alert.dnd-alert');
                ...
                $.each(wrongfiles, function (rkey, result) {
                    ...
                    var alertobj = self.$el.find(
                        '#alertscontent div[tag="'+rkey+'"].alert.dnd-alert');
                    var footer = alertobj.find(
                        'div:last-child span:not(.index-alert)');
                    alertobj.removeClass('alert-danger alert-info').addClass(
                        'alert-'+alert_parts.alerttype);
                    // eslint-disable-next-line max-len
        ...
        removeWrongAlerts: function (alertobj, filekey, removefile) {
            ...
            alertobj.slideUp(500, function () {
                ...
                if (removefile) {
                    ...
                    self.$el.find('#filescontent div[title="'+filekey+'"]')
        ...
        continueAlert: function (alertobj) {
            ...
            /* After the error alert is removed, execute the next actions (Next
                * error alert, Restarts to attach more files, or Shows the
                * final success alert) */
            var self = this;
        ...
        nextWrongAlert: function () {
            ...
            alert.find('div:last-child .index-alert')
                .html(self.alerts_in_queue.total - (keys.length - 1));
            self.$el.find("#alertscontent").html(alert);
            alert.slideDown(500);
        },
        restart: function () {

            /* Restarts all the variables and restores all the DOM element to
                * attach more new files */
            this.files = {};
            this.invoice_ids = [];
            this.uploading_files = false;
        ...
        correctFinalRegistry: function () {
            ...
            /* Shows the final success alert and the button to see the invoices
                * created */
            var self = this;
            // eslint-disable-next-line max-len
