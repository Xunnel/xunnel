    ...
    var attachXmlsWizard = FieldChar.extend({
        ...
        file is not the correct one or the file is already uploaded */
            ...
            if (self.uploading_files) {
                ...
                if (wrong_files.length > 0) {
                    alert_message +=
                        _t("<strong>Info!</strong> You only can upload XML files.<br>") +
                ...
                if (files_used.length > 0) {
                    ...
                    alert_message +=
                        _t("<strong>Info!</strong> Some files are already loaded.<br>") +
                ...
                if ("cfdi_type" in file) {
                    ...
                    var alertelement = $(
                        '<div tag="' +
                            key +
                            '" class="alert alert-' +
                            alert_parts.alerttype +
                            ' dnd-alert">' +
                            alert_parts.errors +
                            "<div>" +
                            alert_parts.buttons +
                            _t('<span>Wrong File: <span class="index-alert"></span>') +
                            "/" +
                            self.alerts_in_queue.total +
                            '<b style="font-size:15px;font-wight:900;">&#8226;</b> ' +
                            key +
                            "</span></div></div>"
                    );
        ...
        prepareWrongAlert: function (key, data) {
            ...
            if (able_buttons.includes("remove")) {
                ...
                buttons +=
                    _t('<button class="dnd-alert-button" tag="remove">Remove XML</button>') +
        ...
        wrongMsgXml: function (file, able_buttons) {
            ...
            var map_error = {
                ...
                version: _t(
                    '<div><span level="1">Unable to generate invoices from an XML with version 3.2.</span>You can create the invoice manually and then attach the xml.</div>'
                ),
                ...
                no_xml_related_uuid: _t(
                    '<div><span level="1">The DocumentType is "E" and The XML UUID / and the node CfdiRelacionados</span> were not found in the XML.</div>'
                ),
            };
            $.each(file, function (ikey, val) {
                ...
                if (ikey === "supplier") {
                    ...
                    errors +=
                        _t('<div><span level="1">Some taxes</span> do not exist: <span level="2">') +
                        val.join(", ") +
                        "</span>.</div>";
                } else if (ikey === "taxes_wn_accounts") {
                    errors +=
                        _t('<div><span level="1">Some taxes</span> do not have account asigned: <span level="2">') +
                        val.join(", ") +
                        "</span>.</div>";
                } else if (ikey === "folio") {
                    errors +=
                    ...
                    errors +=
                        _t(
                            '<div><span level="1">The XML UUID</span> belong to other invoice. <span level="1">UUID: </span>'
                        ) +
                        val +
                        "</div>";
                    ...
                    errors +=
                        _t(
                            '<div><span level="1">The invoice reference</span> belong to other invoice of same partner. <span level="1">Partner: </span>'
                        ) +
                        val[0] +
                    ...
                    errors +=
                        _t(
                            '<div><span level="1">The DocumentType is "E" and The XML UUID</span> is not related to any invoice. <span level="1">UUID: </span>'
                        ) +
                        val +
                        "</div>";
                    $.when(self.getSession().user_has_group("invoice_xunnel.allow_force_invoice_generation")).then(
                        function (has_group) {
                            if (has_group) {
        ...
        removeWrongAlerts: function (alertobj, filekey, removefile) {
            ...
            alertobj.slideUp(500, function () {
                ...
                if (removefile) {
                    ...
                    self.$el
                        .find('#filescontent div[title="' + filekey + '"]')
        ...
        correctFinalRegistry: function () {
            ...
            var self = this;
            var alert = $(
                '<div class="alert alert-success dnd-alert"><strong>' +
                    _t("Congratulations") +
                    "!</strong> " +
                    _t("Your invoices were created correctly") +
                    ".</div>"
            ).hide();
            self.$el.find("#alertscontent").html(alert);
