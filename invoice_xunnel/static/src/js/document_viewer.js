odoo.define("invoice_xunnel.document_viewer", (require) => {
    ...
    DocumentsViewer.include({
        xmlDependencies: ["/invoice_xunnel/static/src/xml/templates.xml"],
        init(parent, documents, activedocumentID) {
            this._super.apply(this, arguments);
            this.document = _.filter(documents, function (document) {
                var match =
                    document.type == "url"
                        ? document.url.match("(youtu|.png|.jpg|.gif)")
                        : document.mimetype.match("(image|video|application/pdf|text)");
                if (match) {
        ...
        _updateContent() {
            ...
            if (xmlViewer.length) {
                ...
                ajax.post(route, {}).then((view) => {
                    ...
                    xmlViewer.find("pre.prettyprint .atn, pre.prettyprint .atv").click(this.copy_attribute.bind(this));
                });
            }
        },
        _onClose(ev) {
