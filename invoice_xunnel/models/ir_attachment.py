...
class IrAttachment(models.Model):
    ...
    def write(self, values):
        ...
        for rec in self:
            ...
            if ("description" in values or "datas" in values) and self._validate_xml(datas):
                ...
                rec_values = values.copy()
                rec_values.update({"description": False, "mimetype": self._compute_mimetype({"datas": datas})})
                super(IrAttachment, rec).write(rec_values)
                no_mx_rec -= rec
    ...
    def _create_description(self, datas):
        ...
        return {
            "description": json.dumps(self._prepare_description_attachment(xml_obj), ensure_ascii=False),
    ...
    def _prepare_description_attachment(self, xml):
        ...
        if hasattr(xml, "Impuestos"):
            data.update(
                {
                    "tax": float(xml.Impuestos.get("TotalImpuestosTrasladados", "0.0")),
                    "tax_retention": float(xml.Impuestos.get("TotalImpuestosRetenidos", "0.0")),
                }
            )
        return data
