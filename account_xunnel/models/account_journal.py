...
class AccountJournal(models.Model):
    ...
    def _compute_has_synchronized_xunnel(self):
        for rec in self:
            ...
            has_online_sync = statement and self.env["account.bank.statement.line"].search(
                [("statement_id", "=", statement.id), ("online_transaction_identifier", "!=", False)], limit=1
            )
    ...
    has_synchronized_xunnel = fields.Boolean(
        help="Recognize if has synchronized with Xunnel", compute="_compute_has_synchronized_xunnel"
    )
