odoo.define("account_xunnel.add_account_manager", (require) => {
    ...
    const AccountManager = Action.extend({
        ...
        start() {
            const deff = this._super.apply(this, arguments).then(() => {
                return this._rpc({
                    ...
                    script.async = false;
                    script.onload = () => {
                        this.$(".__js_dynamic_content__").html(
                            $(qweb.render("account_xunnel.add_account_start_manager"))
                        );
        ...
        openManager() {
            this.$el
                .parents(".modal")
                .addClass("__css_paybook_widget__")
                .find(".modal-content")
                .addClass("__css_paybook_widget_container__");
